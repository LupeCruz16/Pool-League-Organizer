...
public class utility {
    ...
    private static LocalTime convertLocal(String avail){
        if(avail.length() > 1 && Character.isDigit(avail.charAt(1))){//if players availability has a second digit
            ...
            return LocalTime.parse(avail.substring(0, 2) + ":00");//will return double digit times without minutes

            } else {//minutes were found
                return LocalTime.parse(avail.substring(0, 2) + minuteFinder(avail));
            }

        } else {//if player has a single digit availability such as either "2" or "2:30"
            if(minuteFinder(avail) == "0"){//checking to see if minutes were found
            return LocalTime.parse("0" + avail.substring(0, 1) + ":00");//will return single digit times without minutes

            } else {//minutes were found
                return LocalTime.parse("0" + avail.substring(0, 1) + minuteFinder(avail));
            }
